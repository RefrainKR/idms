...
- A successful push to `main`, a completed GitHub Actions run, and the moment the public Pages site actually reflects that deployment are not simultaneous; Pages propagation can take additional time. By default, stop deployment verification after confirming the `main` push succeeded. Do not poll Actions or the public site waiting for propagation. Check the Actions result, deployed asset SHA, or live Pages behavior only when the developer explicitly requests that verification.
- Before any file modification, inspect the current branch. `main` is only a release transit point, and `v.1.<line>.X` is the integration branch for that version line. Normal work starts from an exact released-version branch such as `v.1.12.2`, passes through the applicable integration branch, and uses an exact target-version branch for implementation.
- Project versions use `1.<line>.<patch>`. The leading `1` remains fixed unless the user personally decides that a rewrite-scale change warrants version 2. To avoid ambiguous semantic-version labels, repository instructions should refer to the middle number as the **version line** and the last number as the **patch number**.
- Exact branches such as `v.1.12.2` identify the released version and are the normal resting/check-out point after a successful Pages deployment. A version-line branch such as `v.1.12.X` integrates that line's work; it is not the final checked-out branch after release. `main` represents the currently deployed source and likewise must not remain checked out after release work finishes.
- For a patch release, use this flow: `v.1.12.2` (current release) → `v.1.12.X` (line integration) → `v.1.12.3` (implementation/release candidate) → `v.1.12.X` → `main` → `v.1.12.3` (released resting branch). Increment only the patch number for bug fixes, unintended-result corrections, and small wording/detail changes.
- For a new version line, use this flow: `v.1.12.2` (current release) → `v.1.12.X` → `main` → `v.1.13.X` (new-line development). After the first release in that line is approved and deployed, integrate through `main`, create or update `v.1.13.0` to the deployed commit, and finish checked out on `v.1.13.0`. Increment the middle version-line number for new functionality or intentional material replacement of existing behavior.
- Before entering either flow, fast-forward or merge each transit branch only through the explicit release path; never silently branch new work from a stale exact-version, integration, or `main` pointer. Before declaring deployment complete, confirm that `main`, the applicable `v.1.<line>.X`, and the released exact-version branch point to the intended commits, then check out the exact released-version branch.
...
- GitHub Pages may retain nested ES modules independently, and a query on `main.js` is not inherited by its static imports. Keep repository-source JS/CSS references free of manual `?v=` values. The Pages workflow applies one commit-SHA cache key to every local JS/CSS reference in the runner's temporary deployment source before Jekyll builds it, then rejects missing or mismatched keys in the generated `_site` artifact before deployment. `APP_VERSION` remains the separately managed user-facing application version.
