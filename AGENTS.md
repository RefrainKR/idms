...
- GitHub Pages may retain nested ES modules independently. A query on `main.js` is not inherited by its static imports. When a release changes an imported module API, update the cache-busting query on every changed import edge required to reach that module, and verify the deployed module URLs rather than relying only on a cold local profile.
