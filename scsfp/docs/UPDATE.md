...
샤니송 유틸리티의 업데이트 히스토리를 기록합니다.

이 문서는 개발자와 Codex·Claude 같은 AI 작업자가 과거 변경 내용과 설계 맥락을 확인하기 위한 장기 변경 이력입니다. 날짜가 붙은 과거 항목의 경로·명칭·구현 설명은 당시 상태를 나타내므로 현재 코드에 맞춰 소급 수정하지 않습니다. 현재 동작과 규칙은 실제 소스 코드와 `docs/game/` 문서를 우선하여 확인합니다.
